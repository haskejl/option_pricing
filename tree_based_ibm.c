	...
	for(int t=n-2; t >= 0; t--) {
		for(int i=0; i<mv; i++) {
			for(int j=0; j<mz; j++) {
				...
				St[t][i][j] = exp(-r*dt)*0.25*((1.f+rho)*(nn+pp) + (1.f-rho)*(pn+np));
				//Enable for American Options
				//St[t][i][j] = max(St[t][i][j], max(E-exp(0.5*(z1+z0)), 0.f));
			}
		}
	}
...
int main() {
	...
	#endif
	const double S0 = 83.7;
	const double E[] = {60.f, 70.f, 75.f, 80.f, 85.f, 90.f, 95.f}; 
	...
	for(int i=0; i<7; i++) {
		for(int j=0; j<4; j++) {
			...
			start_t = clock();
			results[i][j] = european_16(S0, E[i], n[j], mv[j], mz[j]);
			end_t = clock();
